...
import { articles, pluralities, words } from './languages/german/words.ts';
import { getRandomItem, getRandomItemWithWeights } from './utils.ts';
import { sentences } from './languages/german/sentences.ts';
...
function getQuestion() {
    ...
    const { sentence, meaning, categories, type } = getRandomItemWithWeights(
        sentences,
        [4],
    );
...
async function main() {
    while (true) {
        // Get a random sentence (add more weight to the simple case)
        const { sentence, meaning, categories, type } =
            getRandomItemWithWeights(sentences, [4]);
        ...
        const answer = await rl.question(
            `${questionSentence} (${translationSentence}) `,
        );
